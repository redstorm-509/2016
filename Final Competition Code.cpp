...
class Robot : public SampleRobot {
	...
	bool cameraenabled = false; // If cameras aren't attached and this is true, it'll break
	bool forward = true; // swap if you want robot to start with other side forward
	float ltrtval = .5;
	float shootspeed = .6;
	int rfminversesafe = -1;
	int rrminversesafe = -1;
	int lfminversesafe = 1;
	int lrminversesafe = 1;
	int rfminverse = rfminversesafe;
	int rrminverse = rrminversesafe;
...
public:
	...
	void jitterbot(bool left){
			...
			if ((left)) {
				rfm.Set((.85) * backwardmultiplier * rfminverse); // left
				rrm.Set((.85) * backwardmultiplier * rrminverse);
				lfm.Set(0);
				lrm.Set(0);
			Wait(0.25);
				lfm.Set((.85/2) * backwardmultiplier * lfminverse);
				lrm.Set((.85/2) * backwardmultiplier * lrminverse);
				rfm.Set(-(.85/2) * backwardmultiplier * rfminverse);
				rrm.Set(-(.85/2) * backwardmultiplier * rrminverse);
			Wait(0.2);
				rfm.Set(-(.85) * backwardmultiplier * rfminverse);
				rrm.Set(-(.85) * backwardmultiplier * rrminverse);
				lfm.Set(0);
				lrm.Set(0);
			Wait(0.25);
			...
			Wait(0.2);
				...
				lfm.Set((.85) * backwardmultiplier * lfminverse); // right
				lrm.Set((.85) * backwardmultiplier * lrminverse);
				rfm.Set(0);
				rrm.Set(0);
			Wait(0.25);
				rfm.Set((.85/2) * backwardmultiplier * rfminverse);
				...
				lrm.Set(-(.85/2) * backwardmultiplier * lrminverse);
			Wait(0.2);
				lfm.Set(-(.85) * backwardmultiplier * lfminverse);
				lrm.Set(-(.85) * backwardmultiplier * lrminverse);
				rfm.Set(0);
				rrm.Set(0);
			Wait(0.25);
		...
		//----------------------------Pivot Turning------------------------//
			if (not(l1)) { // If not var turning. Also can run if trankdrive toggled
					if (l4) { // toggle 90 deg left
						lfm.Set(-.85 * lfminverse);
						lrm.Set(-.85 * lrminverse);
						rfm.Set(.85 * rfminverse);
						rrm.Set(.85 * rrminverse);
						Wait(.35);
					}
					...
					if (l2) { // toggle 180 deg (right)
						lfm.Set(.8 * lfminverse); // era asignado a su propio void
						lrm.Set(.8 * lrminverse);
						rfm.Set(-.8 * rfminverse);
						rrm.Set(-.8 * rrminverse);
						Wait(.7);
					}
				}
		if (not((l1)or (l3))) { // num 2, 4, and 5 stop the program to run
			if ((tankdrive)) { //revisa el bool para ver si tank funciona
			if (r4) { // jitter left
	...
	if (tankdrive) { // override values if it's tank drive
		...
		float rval;
			...
			if (not(forward)) { // if we are toggled backward
				rfm.Set(-lval * rfminverse);
				rrm.Set(-lval * rrminverse);
				lfm.Set(-rval * lfminverse);
				lrm.Set(-rval * lrminverse);
				//SmartDashboard::PutNumber("Left motors",-rval);
				//SmartDashboard::PutNumber("Right motors",-lval);
			}
			if (forward) {
				lfm.Set(lval * lfminverse);
				lrm.Set(lval * lrminverse);
				rfm.Set(rval * rfminverse);
				rrm.Set(rval * rrminverse);
	...
	void OperatorControl() {
		while (IsOperatorControl() && IsEnabled()) {
			rfminverse = rfminversesafe;
					rrminverse = rrminversesafe;
					lfminverse = lfminversesafe;
					lrminverse = lrminversesafe;
					while (IsOperatorControl() && IsEnabled()) {
	...
	void Autonomous() { // Called during autonomous
			...
			donedefence = false;
			if (donedefence) {
				rfm.Set(.6 * rfminverse);
				rrm.Set(.6 * rrminverse);
				lfm.Set(-.6 * lfminverse);
				lrm.Set(-.6 * lrminverse);
				rfminverse *= -1;
				rrminverse *= -1;
				lfminverse *= -1;
				lrminverse *= -1;
				Wait(.5);
				if (lowbar) {
					rfm.Set(.4 * rfminverse);
					rrm.Set(.4 * rrminverse);
					lfm.Set(.25 * lfminverse);
					lrm.Set(.25 * lrminverse);
					Wait(2);
					rfm.Set(.3 * rfminverse);
					rrm.Set(.3 * rrminverse);
					lfm.Set(.3 * lfminverse);
					lrm.Set(.3 * lrminverse);
					Wait(.8);
					rfm.Set(.1 * rfminverse);
					rrm.Set(.1 * rrminverse);
					lfm.Set(.1 * lfminverse);
					lrm.Set(.1 * lrminverse);
				...
				if (pos3) {
					rfm.Set(.3 * rfminverse);
					rrm.Set(.3 * rrminverse);
					lfm.Set(.4 * lfminverse);
					lrm.Set(.4 * lrminverse);
					Wait(2);
					rfm.Set(.1 * rfminverse);
					rrm.Set(.1 * rrminverse);
					lfm.Set(.1 * lfminverse);
				...
				if (pos4) {
					rfm.Set(.25 * rfminverse);
					rrm.Set(.25 * rrminverse);
					lfm.Set(.4 * lfminverse);
					lrm.Set(.4 * lrminverse);
					Wait(2);
					rfm.Set(.1 * rfminverse);
		...
		void lowbarfunc() {
			loadsol.Set(DoubleSolenoid::kReverse);
			Wait(3);
			rfm.Set(.35 * rfminverse);
			rrm.Set(.35 * rrminverse);
			lfm.Set(.35 * lfminverse);
			lrm.Set(.35 * lrminverse);
			Wait(4);
			rfm.Set(0);
			rrm.Set(0);
			lfm.Set(0);
		...
		void chevalfunc() {
			...
			lrm.Set(.3 * lrminverse);
			Wait(1);
			rfm.Set(0 * rfminverse);
			rrm.Set(0 * rrminverse);
			lfm.Set(0 * lfminverse);
			lrm.Set(0 * lrminverse);
			loadsol.Set(DoubleSolenoid::kReverse);
			grabsol.Set(DoubleSolenoid::kReverse);
			Wait(2);
			rfm.Set(.5 * rfminverse);
			rrm.Set(.5 * rrminverse);
			lfm.Set(.5 * lfminverse);
			lrm.Set(.5 * lrminverse);
			loadsol.Set(DoubleSolenoid::kForward);
			grabsol.Set(DoubleSolenoid::kForward);
			Wait(1);
			rfm.Set(0);
		...
		void moatfunc() {
			rfm.Set(.7 * rfminverse);
			rrm.Set(.7 * rrminverse);
			lfm.Set(.7 * lfminverse);
			lrm.Set(.7 * lrminverse);
			Wait(2);
			rfm.Set(.35 * rfminverse);
			rrm.Set(.35 * rrminverse);
			lfm.Set(.35 * lfminverse);
		...
		void rockwallfunc() {
			rfm.Set(.95 * rfminverse);
			rrm.Set(.95 * rrminverse);
			lfm.Set(.95 * lfminverse);
			lrm.Set(.95 * lrminverse);
			Wait(1.2);
			rfm.Set(.2 * rfminverse);
			rrm.Set(.2 * rrminverse);
			lfm.Set(.2 * lfminverse);
			lrm.Set(.2 * lrminverse);
			Wait(2.5);
			rfm.Set(0);
			rrm.Set(0);
			lfm.Set(0);
